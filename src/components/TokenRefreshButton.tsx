...
export default function TokenRefreshButton({ 
  ...
  const { refreshToken, isTokenExpired, isTokenExpiringSoon, isRefreshing } = useTokenRefresh();
  ...
  const getButtonClass = () => {
    const base = 'inline-flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-md transition-colors';
    
    switch (variant) {
      case 'primary':
        return `${base} bg-blue-600 text-white hover:bg-blue-700 disabled:bg-gray-400`;
      case 'secondary':
