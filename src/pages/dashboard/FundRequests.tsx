...
import { useGetAttachmentsQuery, useUploadAttachmentMutation, type Attachment } from '@/api/attachments.api';
...
export default function FundRequests() {
  ...
  const [selectedFundAdjuments, setSelectedFundAdjuments] = useState<FundRequestItem | null>(null);
  ...
  const [createFundAdjuments, { isLoading: isCreating }] = useCreateFundRequestMutation();
  const [updateFundAdjuments, { isLoading: isUpdating }] = useUpdateFundRequestMutation();
  ...
  }, [isCreateModalOpen, isEditMode, time_period, reason, selectedFundAdjuments]);
  ...
  const transformedData: TableRow[] = FundAdjumentsResponse?.results?.map((item: FundRequestItem) => ({
  ...
  const handleStatusClick = (row: TableRow) => {
    console.log("Status clicked for FundAdjuments:", row);
  };
...
// Handler for attachments click
  ...
  const handleDelete = async (row: TableRow) => {
    ...
    if (FundAdjumentsStatus !== 'pending') {
      toast.error(`Cannot delete FundAdjuments with status "${FundAdjumentsStatus}". Only pending FundAdjumentss can be deleted.`);
      return;
    }
  ...
  return (
    <div>
      ...
      <SharedModal
        ...
        title={isEditMode ? 'Edit FundAdjuments Request' : 'Create FundAdjuments Request'}
        size="md"
      >
        <div className="p-4 space-y-4">
          <div>
            <SharedSelect
              key={`transaction-date-${isEditMode ? selectedFundAdjuments?.transaction_id : 'create'}`}
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            <button
              onClick={handleSave}
              disabled={isCreating || isUpdating || !time_period.trim() || !reason.trim()}
              className="px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-blue-600 rounded-md hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
            >
          ...
          >
            <div className="p-4">
              ...
              <div className="space-y-3">
                <h4 className="font-semibold text-gray-800">Existing Attachments</h4>
                        ...
                        <div className="flex items-center gap-3 text-xs text-[#545454]">
                          ...
                          >
                            <svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                              <path d="M8.36902 11.5041C8.27429 11.6077 8.14038 11.6667 8 11.6667C7.85962 11.6667 7.72571 11.6077 7.63099 11.5041L4.96432 8.58738C4.77799 8.38358 4.79215 8.06732 4.99595 7.88099C5.19975 7.69465 5.51602 7.70881 5.70235 7.91262L7.5 9.8788V2.5C7.5 2.22386 7.72386 2 8 2C8.27614 2 8.5 2.22386 8.5 2.5V9.8788L10.2977 7.91262C10.484 7.70881 10.8003 7.69465 11.0041 7.88099C11.2079 8.06732 11.222 8.38358 11.0357 8.58738L8.36902 11.5041Z" fill="#282828"/>
                              <path d="M2.5 10.5C2.5 10.2239 2.27614 10 2 10C1.72386 10 1.5 10.2239 1.5 10.5V10.5366C1.49999 11.4483 1.49998 12.1832 1.57768 12.7612C1.65836 13.3612 1.83096 13.8665 2.23223 14.2678C2.63351 14.669 3.13876 14.8416 3.73883 14.9223C4.31681 15 5.05169 15 5.96342 15H10.0366C10.9483 15 11.6832 15 12.2612 14.9223C12.8612 14.8416 13.3665 14.669 13.7678 14.2678C14.169 13.8665 14.3416 13.3612 14.4223 12.7612C14.5 12.1832 14.5 11.4483 14.5 10.5366V10.5C14.5 10.2239 14.2761 10 14 10C13.7239 10 13.5 10.2239 13.5 10.5C13.5 11.4569 13.4989 12.1244 13.4312 12.6279C13.3655 13.1171 13.2452 13.3762 13.0607 13.5607C12.8762 13.7452 12.6171 13.8655 12.1279 13.9312C11.6244 13.9989 10.9569 14 10 14H6C5.04306 14 4.37565 13.9989 3.87208 13.9312C3.3829 13.8655 3.12385 13.7452 2.93934 13.5607C2.75483 13.3762 2.63453 13.1171 2.56877 12.6279C2.50106 12.1244 2.5 11.4569 2.5 10.5Z" fill="#282828"/>
                            </svg>
                          </button>
                        </div>
                      </div>
    ...
    >
      <div className="flex flex-col items-center gap-6 py-8">
        ...
        <div className="relative bg-[#F6F6F6] p-2 rounded-full">
          <div className="w-16 h-16 border-4 border-[#EFEFEF] border-t-[#0052FF] rounded-full animate-spin"></div>
        </div>
